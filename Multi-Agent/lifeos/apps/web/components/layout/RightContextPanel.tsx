...
export const RightContextPanel: React.FC = () => {
  ...
  return (
    <aside className="fixed right-0 top-16 z-20 flex h-[calc(100vh-4rem)] w-80 flex-col border-l border-border bg-surface p-4 shadow-xl transition-luxury select-none shrink-0">
      <div className="flex items-center justify-between border-b border-border pb-3">
        ...
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-2xl text-text-muted hover:bg-surface-secondary"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="mt-4 space-y-4 overflow-y-auto flex-1">
        ...
        <div className="rounded-2xl border border-border bg-surface-secondary p-3.5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-text-secondary">Orchestrator</span>
            <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-600 dark:text-emerald-400">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> Active
            </span>
          </div>
          <p className="mt-1 text-xs text-text-primary font-semibold">Chief of Staff AI Engine</p>
        </div>
